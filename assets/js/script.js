...
function getData() {
  let ticketMasterURL =
    "https://app.ticketmaster.com/discovery/v2/events.json?classificationName=Sports&apikey=" +
    apiKey +
    "&size=" +
    querySize +
    "&sort=date,asc" +
    "&countryCode=GB,IE,DE,AT,CH,BE,NL,FR,ES,IT,SE,NO,DK,FI,PL,CZ,HU,SK,BR,AR,CL,CA";
  fetch(ticketMasterURL, {
    ...
    .then((data) => {
      ...
      if (events.length) {
        console.log("firsddddddddt", events.length);
        startMapBox(events);
      } else {
...
const startMapBox = async (events) => {
  ...
  map.on("load", async () => {
    ...
    console.log("--------------this is event---------", events.length);
